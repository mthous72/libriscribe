...
export const getActiveModel = (name: string): Promise<{ provider: string, model: string, source: string, configured: boolean }> => api.get(`/projects/${name}/active-model`).then(r => r.data)
